...
file_name=str("GENERATED/"+started_datestring+".txt")
...
# number_of_poems_created_in_this_run=0
...
def sample_model(
    model_name='run2_May6_2019',
    seed=None,
    nsamples=1,
    batch_size=1,
    length=random.randint(60,111),
    temperature=1,
    min_temperature=0.7,
    max_temperature=1.6,
    pause=0,
    ...
    #SLEEP & contemplate time
    start_writing_datestring = "{0:%Y-%m-%d}".format(datetime.now())
    start_writing_timestring = "{0:%H:%M:%S}".format(datetime.now())
    # number_of_poems_created_in_this_run=number_of_poems_created_in_this_run+1
    time.sleep(pause)
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        while nsamples == 0 or generated < nsamples:
            ...
            for i in range(batch_size):
                ...
                # text="ReRites raw output composed by GPT-2 \nfinetuned on a custom contemporary poetry corpus \nwith a "+str(pause)+"sec pause on an Nvidia TitanX \n\n"+ start_writing_datestring+" from " +start_writing_timestring+" - "+"{0:%H-%M-%S}".format(datetime.now())+"\n\nGPT-2 345M model \nretrained for poetry \nfor ReadingRites (2019)\n\nMore info: glia.ca/reritesn\n\nTemperature = "+str(round(temperature,2))+"\n\n\n\n\n\n\t___________________________\n\n\n"+text
                # text="\n\nReRites raw output. \n\n"+start_writing_datestring+"\n" +start_writing_timestring+" - "+"{0:%H:%M:%S}".format(datetime.now())+"\n"+str(pause)+" second pause\n\nGPT-2 345M model \nretrained for poetry \non an Nvidia TitanX \nfor ReadingRites (2019).\n\nMore info: glia.ca/rerites\n\nTemperature = "+str(round(temperature,2))+"\n\n\n\n\n\n\t___________________________\n\n\n"+text

                text=text.replace("\t\t\t","\t    ")
                text="\n\n\n\t    ReRites \n\n\t    [unedited A.I. text] \n\n\t    Date: "+start_writing_datestring+"\n\t    Time: " +start_writing_timestring+" - "+"{0:%H:%M:%S}".format(datetime.now())+"\n\n\t    OpenAI GPT-2 345Mb model \n\t    finetuned with contemporary poetry \n\t    on an Nvidia TitanX\n\n\t    More info: http://glia.ca/rerites\n\t    David Jhave Johnston\n\t    Anteism Books (2019)\n\n\t    Temperature = "+str(round(temperature,2))+"\n\n\n\n\t    ________________________________________________________\n\n\n\n"+text               
